...
function onAssetsLoaded(event) {
	...
	return {
		// This object will be passed to the external `loadedListener` function in the HTML script
		hello: 'world',
	};
}
...
- **sprite.sequence** The current sequence object, defined in sequenceData at creation.
- **sprite.loopCount** The number of loops remaining for the current sequence, 0 means infinite loop
- **sprite.loopDirection** The current loop direction integer, 1 for forward, -1 for reverse.
- **sprite.timeScale** The time scale of the sprite, default is 1, values below 1 will slow down the animation, values above 1 will speed up the animation.
...
Sprites also dispatch the **sprite** event with the following phases:

- _began_: The animation began playing.
- _ended_: The animation showed its final frame.
- _bounce_: The animation bounced from forward to backward while playing.
- _loop_: The animation looped from the beginning of the sequence.
- _next_: The animation played a subsequent frame that’s not one of the above phases.

#### Sprite Example

```javascript
var sequenceData = [{ name: 'idle', start: 0, count: 7, time: 350 }];
...
```javascript
engine.transition.to(object, {
	x: otherObject,
	y: otherObject,
	time: 1000,
	easing: ZetoEasing.outQuad,
	onComplete: function (event) {
